...
static fz_colorspace *fz_default_colorspace(fz_context *ctx, fz_default_colorspaces *default_cs, fz_colorspace *cs)
{
	...
	case FZ_COLORSPACE_MONO:
		if (cs == fz_device_mono(ctx))
			return fz_default_mono(ctx, default_cs);
		break;
	...
	fz_pixmap *dest)
{
	...
	else if (devn && colors_supported(ctx, colorspace, dest))
	{
		...
		//DW
		if (model == fz_device_mono(ctx) && colorfv[0] < 0.80f)
	...
	fz_colorspace *colorspace_in, const float *color, float alpha, fz_color_params color_params)
{
	...
	if (colorspace == fz_device_mono(ctx) && colorbv[0] != 0 && colorbv[0] != 255)
	{
		//If original colorspace is CMYK we may have issues with getting true black or true white.
		//Clip the bottom 18%
		if (colorspace == fz_device_cmyk(ctx) && (colorbv[0] < 179 && colorbv[0] > 46))
			error_diffuse(state->dest, bbox);
		else if (colorspace != fz_device_cmyk(ctx) && colorbv[0] < 179)
			error_diffuse(state->dest, bbox);
		else
...
fz_draw_fill_image(fz_context *ctx, fz_device *devp, fz_image *image, fz_matrix in_ctm, float alpha, fz_color_params color_params)
{
	...
	fz_try(ctx)
	{
		...
		if (model != fz_device_mono(ctx))//DW
		{
		...
		if (conversion_required && after)
		{
			...
			if (state->dest->seps == NULL &&
				(((src_cs == fz_device_gray(ctx) || src_cs == fz_device_mono(ctx)) && model == fz_device_rgb(ctx)) ||
				((src_cs == fz_device_gray(ctx) || src_cs == fz_device_mono(ctx)) && model == fz_device_bgr(ctx))))
			{
		...
		//DW//Paint image first and then do error diffusion!
	...
	fz_try(ctx)
	{
		...
		if (model == fz_device_mono(ctx))
		{
			...
			if (colorbv[0] != 0 && colorbv[0] != 255)
			{
				...
				if (colorspace == fz_device_cmyk(ctx) && (colorbv[0] < 25))
					colorbv[0] = 0;
				else if (colorspace == fz_device_cmyk(ctx) && (colorbv[0] > 230))
					colorbv[0] = 255;
				else
					BlackWhite = 0;
		...
		if (model == fz_device_mono(ctx) && colorbv[0] != 0 && colorbv[0] != 255)
		{
			fz_irect bbox;
			fz_rect urect;
			int ErrorDiffusion = 0;
