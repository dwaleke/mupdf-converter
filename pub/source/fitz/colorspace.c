...
void fz_new_colorspace_context(fz_context *ctx)
{
	...
	fz_try(ctx)
	{
		mono = fz_new_buffer_from_shared_data(ctx, resources_icc_gray_icc, resources_icc_gray_icc_len);
...
int fz_colorspace_is_mono(fz_context* ctx, fz_colorspace* cs)
{
	return cs && cs->type == FZ_COLORSPACE_MONO;
}
...
{
	return fz_colorspace_is_device(ctx, cs) && fz_colorspace_is_mono(ctx, cs);
}
...
fz_new_icc_colorspace(fz_context *ctx, enum fz_colorspace_type type, int flags, const char *name, fz_buffer *buf)
{
	...
	case FZ_COLORSPACE_MONO: return fz_keep_colorspace(ctx, fz_device_mono(ctx));
...
fz_colorspace *fz_new_cal_mono_colorspace(fz_context *ctx, float wp[3], float bp[3], float gamma)
{
	...
	fz_colorspace *cs;
	fz_try(ctx)
		cs = fz_new_icc_colorspace(ctx, FZ_COLORSPACE_MONO, 0, "CalGray", buf);
	fz_always(ctx)
	...
	return cs;
#else
	return fz_keep_colorspace(ctx, fz_device_mono(ctx));
#endif
}
...
fz_default_colorspaces *fz_new_default_colorspaces(fz_context *ctx)
{
	...
	default_cs->refs = 1;
	default_cs->mono = fz_keep_colorspace(ctx, fz_device_mono(ctx));
...
fz_colorspace *fz_default_mono(fz_context *ctx, const fz_default_colorspaces *default_cs)
{
	return default_cs ? default_cs->mono : fz_device_mono(ctx);
}
...
void fz_set_default_mono(fz_context* ctx, fz_default_colorspaces* default_cs, fz_colorspace* cs)
{
	if (cs->type == FZ_COLORSPACE_MONO && cs->n == 1)
	{
		fz_drop_colorspace(ctx, default_cs->mono);
		default_cs->mono = fz_keep_colorspace(ctx, cs);
	}
}
...
fz_init_process_color_converter(fz_context *ctx, fz_color_converter *cc, fz_colorspace *ss, fz_colorspace *ds, fz_colorspace *is, fz_color_params params)
{
	...
	if (ctx->icc_enabled)
	{
		...
		if ((ss->type == FZ_COLORSPACE_GRAY || ss->type == FZ_COLORSPACE_MONO) && (ss->flags & FZ_COLORSPACE_IS_DEVICE))
		{
	...
	int copy_spots)
{
	...
	fz_try(ctx)
	{
		...
		/* Handle DeviceGray to CMYK as K only. See note in Section 6.3 of PDF spec 1.7. */
			...
			((ss->type == FZ_COLORSPACE_GRAY || ss->type == FZ_COLORSPACE_MONO)) &&
